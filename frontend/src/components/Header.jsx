...
const Header = ({ onToggleSidebar }) => {
  ...
  return (
    <header className="w-full bg-gradient-to-r from-indigo-400 via-indigo-500 to-purple-500 shadow-2xl px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between sticky top-0 z-50">
      {/* ✅ Left: Hamburger + Logo + Title */}
      <div className="flex items-center gap-3 sm:gap-4">
        {/* 🍔 Hamburger (YouTube style, left side) */}
        {onToggleSidebar && (
          <button
            onClick={onToggleSidebar}
            className="inline-flex items-center justify-center p-2 rounded-lg bg-white/10 border border-white/20 hover:bg-white/20 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white/40 cursor-pointer"
          >
            <svg
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="hidden xs:block text-2xl sm:text-3xl font-bold text-white tracking-tight cursor-pointer hover:scale-105 transition-transform duration-200">
            Zip Nivasa
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        {/* ✅ USER CARD */}
        <div
          className="relative bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-4 sm:px-5 py-2.5 hover:bg-white/15 transition-all duration-300 cursor-pointer shadow-lg"
            ...
            <div className="absolute top-full right-0 mt-3 w-56 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden animate-fadeIn z-50">
              ...
              <div className="p-2">
                <button
                  ...
                  className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-indigo-50 rounded-lg transition flex items-center gap-3"
                >
                  <svg
        ...
        {/* <button
          onClick={handleLogout}
          className="hidden sm:inline-flex bg-white/10 backdrop-blur-md border border-white/20 text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-white/20 transition-all duration-300 ease-in-out shadow-lg items-center gap-2 hover:scale-105"
        >
